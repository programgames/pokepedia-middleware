from util.connection import session
from pokedex.db.tables import Pokemon, Generation, VersionGroup
from db import pokemon_repository
...
class GenerationHelper:
    ...
    def has_pokemon_availabilities_in_generation(self, pokemon: Pokemon, generation: Generation) -> bool:
        version_groups_entities = session.query(VersionGroup).filter(VersionGroup.generation_id == generation.id).all()

        availabilities = pokemon_repository.is_pokemon_available_in_version_groups(pokemon,
                                                                                   version_groups_entities).all()

        return len(availabilities) > 0

    def get_gen_number_by_name(self, generation: str) -> int:
    ...
    def get_version_group_by_gen_and_column(self, generation: int, column: int) -> VersionGroup:
        col1 = {
            '1': 'red-blue',
            '2': 'gold-silver',
            '3': 'ruby-sapphire',
