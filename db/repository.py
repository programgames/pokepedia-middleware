...
class PokemonRepository:
    ...
    def find_default_gen8_pokemons(self):
        national_pokedex = session.query(Pokedex).filter(Pokedex.identifier == 'national').one()
        species_results = session.query(PokemonSpecies.id) \
            ...
            .filter(Pokedex.id == national_pokedex.id) \
            .filter(PokemonDexNumber.pokedex_number.in_(self.get_default_gen8_national_dex_pokemon_number())) \
            .all()
    ...
    def find_pokemon_by_identifier(self, name: str) -> Pokemon:
        return session.query(Pokemon).filter(Pokemon.identifier == name).one()

    def find_pokemon_with_specific_page(self, start_at: int):
...
class PokemonMoveAvailabilityRepository:
    def __init(self):
        pass

    def find_availability_by_pkm_and_form(self, name: str, version_group: VersionGroup) -> PokemonMoveAvailability:
        return session.query(PokemonMoveAvailability) \
...
pokemon_move_availability_repository = PokemonMoveAvailabilityRepository()
