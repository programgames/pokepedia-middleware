...
from middleware.satanizer import pokepedialevelmovesatanizer, pokepediamachinemovesatanizer
...
def get_pokemon_moves(name: str, generation: int, method_type: str) -> dict:
    if method_type == LEVELING_UP_TYPE:
        ...
        if generation >= 1 <= 6:
            moves_data = _get_pokemon_moves_from_cache(name, generation, method_type)
            moves_data['satanized'] = pokepediamachinemovesatanizer.check_and_sanitize_moves(moves_data['wikitext'],
                                                                                             name)
        else:
