...
def get_move_french_name(move: Move):
    return move.movename.get(language_id=french.id).name
...
def get_pokemon_specy_french_name(specy: PokemonSpecies):
    return specy.pokemonspeciesname.get(language_id=french.id).name

def get_pokemon_form_french_name(pokemon_form: PokemonForm):
    return pokemon_form.pokemonformname.get(language_id=french.id).name
